	...
	return new Promise(function(resolve, reject) {
		...
		if (items.bolt && items.routing) {
			var driver = neo4j.driver(
				`bolt+routing://${items.ip}`,
				neo4j.auth.basic(items.user, items.password)
			);
		} else if (items.bolt) {
			var driver = neo4j.driver(
				`bolt://${items.ip}`,
		...
		session
			...
			.then(res => {
				...
				session
					.readTransaction(tx => tx.run("CALL dbms.cluster.role()"))
					.then(res => {
						res.records.forEach(function(record) {
							if (record.keys && record._fields) {
								senditem.clusterReadHealth = "Working";
								if (
									record._fields[0] == "FOLLOWER" ||
									record._fields[0] == "READ_REPLICA"
								) {
